...
void CMSOpenDataTrigger::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int i = 0; i < triggersThatMatter.size(); i++) {
    ...
    cout << eventNumber << "," << runNumber << "," << name << "," << fired << "," << prescale.first << "," << prescale.second << endl;
    csvOut_ << eventNumber << "," << runNumber << "," << name << "," << fired << "," << prescale.first << "," << prescale.second << endl;
  }
